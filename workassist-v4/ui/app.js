// WorkAssist v4 - HTTP Backend API Adapter (Replacing Tauri IPC)
async function invoke(cmd, args = {}) {
    try {
        ...
        console.error(`[API Error] ${cmd}:`, err);
        throw err;
    }
}
console.log("WorkAssist v4 HTTP API Adapter Initialized");

// --- Centralized Language & UI Strings Constant Dictionary (I18N) ---
const I18N = {
    days: ['일', '월', '화', '수', '목', '금', '토'],
    ...
    common: {
        ...
        exportSuccess: (path) => `내보내기가 완료되었습니다!\n저장 경로: ${path}`,
        exportFailed: (err) => `내보내기 실패: ${err}`,
        importSuccess: (msg) => `가져오기가 완료되었습니다: ${msg}`,
        importFailed: (err) => `가져오기 실패: ${err}`,
        desktopOnly: '데스크톱 앱 환경에서만 지원되는 기능입니다.',
        selectProjectFirst: '프로젝트를 먼저 선택해 주세요.'
    },
    auth: {
        requireCredentials: '아이디와 비밀번호를 모두 입력해 주세요.',
        invalidCredentials: '잘못된 아이디 또는 비밀번호입니다.',
        loginFailed: (err) => `로그인에 실패했습니다: ${err}`,
        requireId: '아이디를 입력해 주세요.',
        hintNotFound: '해당 사용자의 힌트를 찾을 수 없습니다.',
        hintError: '힌트를 가져오는 중 오류가 발생했습니다.',
        registerSuccess: '계정이 성공적으로 생성되었습니다!',
        registerFailed: (err) => `계정 생성에 실패했습니다: ${err}`,
        passwordChanged: '비밀번호가 변경되었습니다!',
        passwordChangeFailed: (err) => `비밀번호 변경에 실패했습니다: ${err}`
    },
    validation: {
        dateOrderError: '마감일은 시작일 이후로 설정해야 합니다.',
        requiredFields: '모든 필드(제목, 내용, 담당자, 시작일, 마감일)를 작성해 주세요.',
        titleLength: (max) => `제목은 ${max}자 이하여야 합니다.`,
        contentLength: (max) => `내용은 ${max}자 이하여야 합니다.`,
        assigneeLength: (max) => `담당자는 ${max}자 이하여야 합니다.`,
        locationLength: (max) => `장소는 ${max}자 이하여야 합니다.`
    },
    kanban: {
        totalLimitExceeded: '전체 업무 개수 제한 초과: 최대 200개의 업무만 등록할 수 있습니다.',
        statusLimitExceeded: (status) => `상태별 업무 개수 제한 초과: '${status}' 상태에는 최대 50개의 업무만 담을 수 있습니다.`,
        dateLimitExceeded: (date) => `일자별 업무 개수 제한 초과: 해당 시작일(${date})에는 최대 20개의 업무만 등록할 수 있습니다.`,
        noScheduleTasks: '스케줄에 표시할 업무가 없습니다.',
        noTasks: '등록된 업무가 없습니다.',
        restoreConfirm: '이 업무를 원래 상태로 복원하시겠습니까?',
        deleteConfirm: '이 업무를 영구적으로 삭제하시겠습니까? 이 작업은 취소할 수 없습니다.'
    },
    pm: {
        noActiveProject: '선택된 프로젝트가 없습니다.',
        noActiveLogs: '이 프로젝트에 등록된 활성 상태 로그가 없습니다.',
        noPlannedDays: '프로젝트 로그에 계획된 일수가 없습니다. (시작/마감일 미설정)',
        noMinutes: '등록된 회의록이 없습니다.',
        noProjects: '진행 중인 프로젝트가 없습니다.',
        selectProject: '프로젝트를 선택하세요',
        deleteConfirm: (name) => `"${name}" 프로젝트 및 하위 일정을 휴지통으로 이동하시겠습니까?`,
        exportSuccess: '프로젝트 DB가 성공적으로 내보내졌습니다!',
        exportHtmlSuccess: (path) => `프로젝트 HTML 내보내기 성공!\n저장 경로: ${path}`,
        exportMdSuccess: (path) => `프로젝트 마크다운 내보내기 성공!\n저장 경로: ${path}`
    },
    minutes: {
        noMinutes: '등록된 회의록이 없습니다.',
        noMinutesCategory: '이 카테고리에 등록된 회의록이 없습니다.',
        noCategories: '생성된 카테고리가 없습니다.',
        catNameEmpty: '카테고리 이름을 입력해 주세요.',
        catNameLength: '카테고리 이름은 15자 이하여야 합니다.',
        catExists: (name) => `"${name}" 이름의 카테고리가 이미 존재합니다.`,
        catDeleteConfirm: (name) => `"${name}" 카테고리를 삭제하시겠습니까?\n연관된 회의록은 미분류 상태로 전환됩니다.`,
        catLimit: '카테고리는 최대 50개까지 추가할 수 있습니다.',
        catMeetingLimit: '카테고리당 최대 99개의 회의록만 저장할 수 있습니다.',
        requireTitle: '내보내기 전에 회의 제목을 입력해 주세요.',
        restoreConfirm: '이 회의록을 복원하시겠습니까?',
        deleteConfirm: '이 회의록을 영구적으로 삭제하시겠습니까?'
    },
    trash: {
        noTasks: '삭제된 업무 목록이 없습니다.',
        noMeetings: '삭제된 회의록이 없습니다.',
        noProjects: '삭제된 프로젝트가 없습니다.',
        restoreSuccess: '성공적으로 복원되었습니다.',
        deleteSuccess: '성공적으로 영구 삭제되었습니다.'
    }
};
...
async function loadAppVersion() {
    try {
        if (invoke) {
            currentAppVersion = await invoke('plugin:engine|get_app_version');
        }
    } catch (err) {
    ...
    if (currentAppVersion) {
        const verElements = document.querySelectorAll('.app-version-text');
        verElements.forEach(el => {
            el.textContent = `v${currentAppVersion}`;
        });
    }
}
...
const init = async () => {
    ...
    navItems.forEach(item => {
        item.addEventListener('click', (e) => {
            ...
            currentView = item.id;
            const navTextElem = item.querySelector('.nav-text');
            const viewName = navTextElem ? navTextElem.textContent.trim() : item.textContent.trim();
    ...
    const setSidebarCollapsed = (collapsed) => {
        ...
        if (collapsed) {
            ...
            if (btnToggleSidebar) btnToggleSidebar.title = '사이드메뉴 펼침';
        } else {
            sidebar.classList.remove('collapsed');
            if (sidebarToggleIcon) sidebarToggleIcon.textContent = '◀';
            if (btnToggleSidebar) btnToggleSidebar.title = '사이드메뉴 접기';
        }
...
function setupModals() {
    ...
    const btnViewProjectOverview = document.getElementById('btn-view-project-overview');
    const modalProjectOverview = document.getElementById('modal-project-overview');
    const btnCloseProjectOverview = document.getElementById('btn-close-project-overview');
    const btnCloseProjectOverviewFooter = document.getElementById('btn-close-project-overview-footer');

    if (btnViewProjectOverview) {
        btnViewProjectOverview.onclick = () => {
            if (!currentProjectId) {
                alert(I18N.pm.noActiveProject);
                return;
            }
            if (modalProjectOverview) modalProjectOverview.classList.remove('hidden');
        };
    }
    if (btnCloseProjectOverview) {
        btnCloseProjectOverview.onclick = () => {
            if (modalProjectOverview) modalProjectOverview.classList.add('hidden');
        };
    }
    if (btnCloseProjectOverviewFooter) {
        btnCloseProjectOverviewFooter.onclick = () => {
            if (modalProjectOverview) modalProjectOverview.classList.add('hidden');
        };
    }
    if (formCompleteProject) {
        formCompleteProject.onsubmit = async (e) => {
            ...
            try {
                ...
                const overviewDesc = document.getElementById('overview-modal-project-desc');
                if (overviewDesc) overviewDesc.textContent = '설명이 없습니다.';
                ...
                const btnExportDb = document.getElementById('pm-export-db');
                if (btnExportDb) btnExportDb.classList.add('hidden');
    ...
    const btnActiveProjectsModal = document.getElementById('btn-active-projects-modal');
    const modalActiveProjects = document.getElementById('modal-active-projects');
    const btnCloseActiveProjects = document.getElementById('btn-close-active-projects');
    const btnCloseActiveProjectsFooter = document.getElementById('btn-close-active-projects-footer');

    if (btnActiveProjectsModal) {
        btnActiveProjectsModal.onclick = () => {
            if (modalActiveProjects) modalActiveProjects.classList.remove('hidden');
        };
    }
    if (btnCloseActiveProjects) {
        btnCloseActiveProjects.onclick = () => {
            if (modalActiveProjects) modalActiveProjects.classList.add('hidden');
        };
    }
    if (btnCloseActiveProjectsFooter) {
        btnCloseActiveProjectsFooter.onclick = () => {
            if (modalActiveProjects) modalActiveProjects.classList.add('hidden');
        };
    }
    if (btnAddCategory) {
    ...
    if (btnExportMd) {
        btnExportMd.onclick = async () => {
            ...
            if (!meetingData.title) {
                alert(I18N.minutes.requireTitle);
                return;
            }
            ...
            try {
                ...
                if (window.__TAURI__ && window.__TAURI__.dialog) {
                    ...
                    if (savePath) {
                        ...
                        alert(I18N.common.exportSuccess(savePath));
                    }
                } else {
                    console.log("Mock Export (No Tauri):", mdContent);
                    alert(I18N.common.desktopOnly);
                }
            } catch (err) {
                console.error("Export Error:", err);
                alert(I18N.common.exportFailed(err));
            }
        };
    }
    // Privacy Modal
                ...
                btnDelProjectModal.onclick = async () => {
                    if (await askConfirm('이 프로젝트와 모든 관련 이정표(Milestones) 및 로그들을 휴지통으로 이동하시겠습니까?')) {
                        try {
                            ...
                            document.getElementById('detail-project-name').textContent = '프로젝트를 선택하세요';
                            document.getElementById('detail-project-meta').textContent = '고객사: - | 담당자: - | 시작일: -';
                            const overviewDesc2 = document.getElementById('overview-modal-project-desc');
                            if (overviewDesc2) overviewDesc2.textContent = '설명이 없습니다.';
                            document.getElementById('detail-project-tag').classList.add('hidden');
                            const btnEditProject = document.getElementById('btn-edit-project');
                            if (btnEditProject) btnEditProject.classList.add('hidden');
                            const btnExportHtml = document.getElementById('pm-timetable-export-html');
    ...
    if (formLog) {
        formLog.onsubmit = async (e) => {
            ...
            try {
                ...
                await loadProjectDetails(log.project_id); // Refresh project details
    ...
    if (btnExportHtml) {
        btnExportHtml.onclick = async () => {
            if (!currentProjectId) {
                alert(I18N.common.selectProjectFirst);
                return;
            }
            ...
            try {
                ...
                const htmlContent = generateProjectHtml(project, milestones, logs, timetableBaseDate);
                ...
                if (window.__TAURI__ && window.__TAURI__.dialog) {
                    ...
                    if (savePath) {
                        ...
                        alert(I18N.pm.exportHtmlSuccess(savePath));
                    }
                } else {
    ...
    const btnPmExportDb = document.getElementById('pm-export-db');
    if (btnPmExportDb) {
        btnPmExportDb.onclick = async () => {
            if (!currentProjectId) {
            ...
            try {
                if (window.__TAURI__ && window.__TAURI__.dialog) {
                    const savePath = await window.__TAURI__.dialog.save({
                        title: 'Export Project DB File',
                        filters: [{ name: 'Project DB File', extensions: ['json'] }],
                        defaultPath: `Project_DB_${project.project_tag || project.name}_${project.name}.json`
                    });

                    if (savePath) {
                        await invoke('plugin:pm|export_project_db', {
                            projectId: currentProjectId,
                            filePath: savePath
                        });
                        alert(`프로젝트 DB가 성공적으로 내보내졌습니다.\n저장 경로: ${savePath}`);
                    }
                } else {
                    alert("데스크톱 앱 환경에서만 지원되는 기능입니다.");
                }
            } catch (err) {
                console.error("Export Project DB Error:", err);
                alert("프로젝트 DB 내보내기 실패: " + err);
            }
        };
    }
...
function setupSettings() {
        ...
        btnManualBackup.onclick = async () => {
            try {
                const path = await window.__TAURI__.dialog.save({
                    filters: [{ name: 'SQLite Database', extensions: ['db'] }],
                    defaultPath: 'workassist_v3_manual_backup.db'
                });
    ...
    if (btnInitializeDb) {
        btnInitializeDb.onclick = async () => {
            if (await askConfirm("데이터를 초기화하시겠습니까? 모든 데이터가 영구적으로 삭제됩니다!")) {
                try {
...
async function loadDeletedTasks() {
    ...
    try {
        ...
        if (deletedTasks.length === 0) {
            body.innerHTML = '<tr><td colspan="7" style="text-align:center; padding: 40px; color: var(--text-muted)">삭제된 업무가 없습니다.</td></tr>';
            return;
        }
            ...
            row.innerHTML = `
                ...
                <td>
                    <div class="action-buttons">
                        <button class="btn-restore" data-id="${t.id}">🔄 복원</button>
                        <button class="btn-hard-del" data-id="${t.id}">🔥 영구 삭제</button>
                    </div>
                </td>
            `;
        ...
        body.querySelectorAll('.btn-hard-del').forEach(btn => {
            btn.onclick = async (e) => {
                e.stopPropagation();
                if (await askConfirm('이 업무를 영구적으로 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.')) {
...
function drawDepartmentWorkload(containerId, selectedProjItem) {
    ...
    if (totalVal === 0) {
        const emptyMsg = metric === 'days' 
            ? '프로젝트 로그에 계획된 일수가 없습니다. (시작/마감일 미설정)'
            : '이 프로젝트에 등록된 활성 상태 로그가 없습니다.';
        container.innerHTML = `
            <div style="text-align: center; color: var(--text-secondary); font-size: 13px; padding: 20px 0;">
                ${emptyMsg}
            </div>
    ...
    Object.keys(depts).forEach(key => {
        ...
        const labelStr = metric === 'days' ? `${dept.value}일` : `${dept.value}개 로그`;
        const tooltipStr = metric === 'days' 
            ? `${dept.label}: 계획된 ${dept.value}일 (전체 일정의 ${pct}%)`
            : `${dept.label}: 활성 상태 로그 ${dept.value}개 (전체 업무의 ${pct}%)`;
...
function drawMeetingCategories(containerId, meetings, categories) {
    ...
    categoryCounts[0] = { name: '미분류', color: '#64748b', count: 0 };
    ...
    sortedCats.forEach(cat => {
        ...
        item.innerHTML = `
            ...
            <div class="category-chart-right">
                <span class="category-chart-count">${cat.count}건</span>
                <span class="category-chart-pct">${pct}%</span>
            </div>
        `;

        item.onmouseenter = (e) => {
            showChartTooltip(e, `카테고리 "${cat.name}": ${cat.count}개 회의록 (전체의 ${pct}%)`);
        };
        item.onmouseleave = () => {
            hideChartTooltip();
        };
...
function updateSidebarStatus(tasks) {
    ...
    if (statusEl) {
        const verStr = currentAppVersion ? `v${currentAppVersion}` : 'v3.0.0';
        statusEl.textContent = `${verStr} | ${pendingCount} Pending`;
    }
}
...
function renderSchedule() {
    ...
    if (currentTasks.length === 0) {
        scheduleBody.innerHTML = '<div style="text-align: center; padding: 40px; color: var(--text-secondary)">스케줄에 표시할 업무가 없습니다.</div>';
        return;
    }
    ...
    lanesToRender.forEach(lane => {
        ...
        laneEl.innerHTML = `
            ...
            <div class="lane-import-actions" style="display: flex; gap: 8px; padding: 0 2px;">
                <button class="btn btn-outline-accent btn-lane-import-single" style="flex: 1; font-size: 11px; padding: 6px 4px; line-height: 1.2; height: auto; display: flex; align-items: center; justify-content: center; gap: 4px; border-radius: 6px;" title="파일에서 MD 가져오기">
                    <span>📄</span> 파일 가져오기
                </button>
                <button class="btn btn-outline-accent btn-lane-import-bulk" style="flex: 1; font-size: 11px; padding: 6px 4px; line-height: 1.2; height: auto; display: flex; align-items: center; justify-content: center; gap: 4px; border-radius: 6px;" title="폴더에서 MD 가져오기">
                    <span>📁</span> 폴더 가져오기
                </button>
            </div>
        ...
        btnSingle.onclick = async (e) => {
            ...
            try {
                const file = await window.__TAURI__.dialog.open({
                    directory: false,
                    multiple: false,
                    title: `"${lane.name}" 카테고리로 가져올 마크다운 파일 선택`,
                    filters: [{ name: 'Markdown', extensions: ['md'] }]
                });
        ...
        if (lane.meetings.length === 0) {
            ...
            emptyEl.textContent = '이 카테고리에 등록된 회의록이 없습니다.';
            ...
            card.innerHTML = `
                ...
                <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-top: auto;">
                    <div class="meeting-info" style="margin-top: 0;">
                        <span>📍 ${m.location || '원격'}</span>
                        <span>👥 ${m.participants || '없음'}</span>
                    </div>
                    ${catBadge}
                </div>
                <div class="meeting-actions-mini">
                    <button class="btn-icon delete-meeting-btn" title="삭제">
                        🗑️
                    </button>
                </div>
            `;
...
async function loadDeletedMeetings() {
    ...
    try {
        ...
        if (deletedMeetings.length === 0) {
            body.innerHTML = '<tr><td colspan="6" style="text-align:center; padding: 40px; color: var(--text-muted)">삭제된 회의록이 없습니다.</td></tr>';
            return;
        }
        deletedMeetings.forEach(m => {
            ...
            row.innerHTML = `
                ...
                <td>
                    <div class="action-buttons">
                        <button class="btn-restore" data-id="${m.id}">🔄 복원</button>
                        <button class="btn-hard-del" data-id="${m.id}">🔥 영구 삭제</button>
                    </div>
                </td>
            `;
            body.appendChild(row);
        ...
        body.querySelectorAll('.btn-hard-del').forEach(btn => {
            btn.onclick = async (e) => {
                e.stopPropagation();
                if (await askConfirm('이 회의록을 영구적으로 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.')) {
...
async function refreshProjects() {
    try {
        ...
        // Update header & modal count badges
        const btnCount = document.getElementById('btn-active-projects-count');
        if (btnCount) btnCount.textContent = currentProjects.length;

        const modalBadge = document.getElementById('active-projects-count-badge');
        if (modalBadge) modalBadge.textContent = `${currentProjects.length}개`;
        ...
        if (currentProjects.length === 0) {
            list.innerHTML = '<div style="text-align: center; color: var(--text-secondary); padding: 40px;">진행 중인 프로젝트가 없습니다.</div>';
            return;
        }
        ...
        currentProjects.forEach(p => {
            ...
            const formattedDate = p.start_date ? `📅 시작일: ${p.start_date}` : '📅 시작일: -';
            
            item.innerHTML = `
                ${tagBadge}
                <h4>${p.name}</h4>
                <div class="client">${p.client ? `고객사: ${p.client}` : '자사 프로젝트'}</div>
                <div class="project-start-date">${formattedDate}</div>
                <div class="project-actions-group">
                    <button class="btn-icon complete-project-btn" title="프로젝트 완료">✓</button>
                    <button class="btn-icon export-project-btn" title="DB 내보내기">📤</button>
                    <button class="btn-icon delete-project-btn" title="삭제">🗑️</button>
                </div>
            `;
            item.addEventListener('click', () => {
                loadProjectDetails(p.id);
                const modal = document.getElementById('modal-active-projects');
            ...
            if (delBtn) {
                delBtn.onclick = async (e) => {
                    e.stopPropagation();
                    if (await askConfirm(`"${p.name}" 프로젝트 및 하위 일정을 휴지통으로 이동하시겠습니까?`)) {
                        try {
                            await invoke('plugin:pm|delete_project', { projectId: p.id });
                            ...
                            if (isActiveSelected) {
                                document.getElementById('detail-project-name').textContent = '프로젝트를 선택하세요';
                                document.getElementById('detail-project-meta').textContent = '고객사: - | 담당자: - | 시작일: -';
                                const overviewDesc3 = document.getElementById('overview-modal-project-desc');
                                if (overviewDesc3) overviewDesc3.textContent = '선택된 프로젝트가 없습니다.';
                                document.getElementById('detail-project-tag').classList.add('hidden');
                                const btnEditProject = document.getElementById('btn-edit-project');
                                if (btnEditProject) btnEditProject.classList.add('hidden');
                                const btnExportHtml = document.getElementById('pm-timetable-export-html');
...
async function loadProjectDetails(projectId) {
    ...
    const btnExportDb = document.getElementById('pm-export-db');
    if (btnExportDb) {
        btnExportDb.classList.remove('hidden');
    }
    const metaText = `고객사: ${project.client || '-'} | 담당자: ${project.manager || '-'} | 시작일: ${project.start_date || '-'}`;
    document.getElementById('detail-project-meta').textContent = metaText;

    // Update Overview Modal Elements
    const overviewName = document.getElementById('overview-modal-project-name');
    if (overviewName) overviewName.textContent = project.name;
    const overviewMeta = document.getElementById('overview-modal-project-meta');
    if (overviewMeta) overviewMeta.textContent = metaText;
    const overviewDesc = document.getElementById('overview-modal-project-desc');
    if (overviewDesc) overviewDesc.textContent = project.description || '설명이 없습니다.';
...
async function renderTimeTable() {
    ...
    function calculatePct(dateInput) {
    ...
    msLinesContainer.style.width = 'calc(100% - 140px)';
    msLinesContainer.style.minWidth = `calc(${days} * 80px)`;
    ...
    depts.forEach(dept => {
        ...
        if (dept.key === 'Milestone') {
            ...
            // Filter status logs by matching the department value (case-insensitive & trimmed)
            let deptLogs = logs.filter(l => (l.department || '').trim().toLowerCase() === (dept.name || '').trim().toLowerCase());
            if (!showDeleted) {
...
function renderStatusGrid(project) {
    ...
    currentStatusLogs.forEach(log => {
        ...
        const normDept = (log.department || '').trim().toLowerCase();
        if (normDept === slot1.trim().toLowerCase()) { targetEl = colListMech; slotColor = '#3b82f6'; }
        else if (normDept === slot2.trim().toLowerCase()) { targetEl = colListControl; slotColor = '#f59e0b'; }
        else if (normDept === slot3.trim().toLowerCase()) { targetEl = colListElec; slotColor = '#10b981'; }
        else if (normDept === slot4.trim().toLowerCase()) { targetEl = colListSales; slotColor = '#ef4444'; }
        else if (normDept === slot5.trim().toLowerCase()) { targetEl = colListDept5; slotColor = '#8b5cf6'; }
        else if (normDept === slot6.trim().toLowerCase()) { targetEl = colListDept6; slotColor = '#ec4899'; }
        else if (normDept === slot7.trim().toLowerCase()) { targetEl = colListDept7; slotColor = '#06b6d4'; }
        else if (normDept === slot8.trim().toLowerCase()) { targetEl = colListDept8; slotColor = '#eab308'; }
        else if (normDept === slot9.trim().toLowerCase()) { targetEl = colListDept9; slotColor = '#14b8a6'; }
        else if (normDept === slot10.trim().toLowerCase()) { targetEl = colListDept10; slotColor = '#a855f7'; }
...
function generateProjectHtml(project, milestones, logs, currentBaseDate) {
    const baseDateISO = currentBaseDate ? currentBaseDate.toISOString() : new Date().toISOString();
    ...
    <style>
        * {
        ...
        .timetable-header, .timetable-body {
            min-width: calc(140px + var(--day-count, 12) * 80px);
        }
        .timetable-header {
            display: flex;
        ...
        .timetable-x-axis {
            display: flex;
            flex: 1;
            min-width: calc(var(--day-count, 12) * 80px);
            position: relative;
        }
        .timetable-day-col {
            flex: 1;
        ...
        @media print {
            ...
            .timetable-header, .timetable-body {
                min-width: calc(140px + var(--day-count, 12) * 80px) !important;
                width: 100% !important;
            }
            .timetable-header {
...
<body>
    ...
    <script>
        ...
        let timetableBaseDate = new Date("${baseDateISO}");
        ...
        function calculatePct(dateInput, startDate, scale) {
            const date = parseLocalDate(dateInput);
            if (!date || isNaN(date.getTime())) return -1;
            if (scale === 'weekly') {
                const diffTime = date.getTime() - startDate.getTime();
        ...
        function renderTimeTable() {
            ...
            if (milestones.length > 0) {
                const sorted = [...milestones].filter(m => m.is_saved && m.deadline).sort((a, b) => (parseLocalDate(a.deadline) || 0) - (parseLocalDate(b.deadline) || 0));
                sorted.forEach((m) => {
                    const mDate = parseLocalDate(m.deadline);
            ...
            depts.forEach(dept => {
                ...
                deptLogs.sort((a, b) => (parseLocalDate(a.timestamp) || 0) - (parseLocalDate(b.timestamp) || 0));
                ...
                deptLogs.forEach(log => {
                    ...
                    if (sDateStr && dDateStr && sDateStr !== dDateStr) {
                        const s = parseLocalDate(sDateStr);
                        const d = parseLocalDate(dDateStr);
                        ...
                        // Point display (uses due_date/deadline if present, else start_date, else timestamp)
                        const targetDate = dDateStr ? parseLocalDate(dDateStr) : (sDateStr ? parseLocalDate(sDateStr) : logDate);
...
function checkTaskLimits(task, isNew) {
    ...
    if (isNew && activeTasks.length >= 200) {
        alert(I18N.kanban.totalLimitExceeded);
        return false;
    }
    ...
    if (sameStatusCount >= 50) {
        alert(I18N.kanban.statusLimitExceeded(status));
        return false;
    }
    ...
    if (task.start_date) {
        ...
        if (sameDateCount >= 20) {
            alert(I18N.kanban.dateLimitExceeded(task.start_date));
            return false;
        }
    }
...
async function refreshCategories() {
    try {
        ...
        if (editorSelect) {
            ...
            editorSelect.innerHTML = `<option value="">${I18N.common.uncategorized}</option>`;
        ...
        if (filterSelect) {
            ...
            filterSelect.innerHTML = `
                <option value="all">${I18N.common.allCategories}</option>
                <option value="none">${I18N.common.uncategorized}</option>
            `;
            currentCategories.forEach(cat => {
                const opt = document.createElement('option');
                opt.value = cat.id;
...
function renderCategoriesList() {
    ...
    if (currentCategories.length === 0) {
        container.innerHTML = '<div style="text-align: center; color: var(--text-secondary); padding: 20px;">생성된 카테고리가 없습니다.</div>';
        return;
    }
    ...
    currentCategories.forEach(cat => {
        ...
        if (editBtn) {
            editBtn.onclick = (e) => {
                ...
                const saveFn = async () => {
                    ...
                    if (!newName) {
                        alert(I18N.minutes.catNameEmpty);
                        return;
                    }
                    
                    if (newName.length > 15) {
                        alert(I18N.minutes.catNameLength);
                        return;
                    }
                    ...
                    if (exists) {
                        alert(I18N.minutes.catExists(newName));
                        return;
                    }
        ...
        if (delBtn) {
            delBtn.onclick = async () => {
                if (await askConfirm(I18N.minutes.catDeleteConfirm(cat.name))) {
                    try {
...
function setupRag() {
    ...
    // Index parsed JSON content to SQLite
...
// ===== HISTORY MANAGER (D3.js Knowledge Graph) =====
        ...
        try {
            ...
            if (btnDelProj) {
                btnDelProj.onclick = async () => {
                    const confirmDel = confirm(`프로젝트 '${d.label}'와 관련된 엔티티 데이터를 모두 완전히 영구 삭제하시겠습니까?`);
                    if (confirmDel) {
                        try {
    ...
    if (btnConfirmIngest) {
        btnConfirmIngest.addEventListener('click', async () => {
            ...
            try {
                ...
                if (successCount > 0) {
                    writeDbLog(`인입 완료! SQLite DB 적재 성공: ${successCount}건, 실패: ${failCount}건.`);
