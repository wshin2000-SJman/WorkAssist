...
impl Api {
    ...
    pub fn import_db(&self, path: String, user: Option<User>) -> Result<(), String> {
        self.storage.import_database(std::path::PathBuf::from(path), user)
    }

    pub fn initialize_data(&self, user: Option<User>) -> Result<(), String> {
        self.storage.initialize_all_data(user).map_err(|e| e.to_string())
    }
}
